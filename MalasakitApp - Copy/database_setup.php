...
$sql = "CREATE TABLE IF NOT EXISTS users (
    ...
    allergies TEXT,
    avatar VARCHAR(255),
    user_type ENUM('patient', 'health_worker') DEFAULT 'patient',
...
if ($result->num_rows == 0) {
    $conn->query("ALTER TABLE users ADD COLUMN user_type ENUM('patient', 'health_worker') DEFAULT 'patient'");
}

// Add specialty column if it doesn't exist
$result = $conn->query("SHOW COLUMNS FROM users LIKE 'specialty'");
if ($result->num_rows == 0) {
    $conn->query("ALTER TABLE users ADD COLUMN specialty VARCHAR(100)");
}

echo "User type columns added<br>";
...
// Create messages table - links patients and health workers through users table
// Drop existing table if exists to update structure
$conn->query("DROP TABLE IF EXISTS messages");
$sql = "CREATE TABLE IF NOT EXISTS messages (
    id INT AUTO_INCREMENT PRIMARY KEY,
    patient_id INT NOT NULL,
    health_worker_id INT NOT NULL,
    sender_type ENUM('patient','health_worker') NOT NULL,
