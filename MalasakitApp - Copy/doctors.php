...
// Fetch health workers from database - combine health_workers table AND users table
$workers = [];

// First get from health_workers table
...
// Also get health workers from users table (registered through signup)
$sql = "SELECT id, fullname as name, specialty, phone, email, avatar, user_type FROM users WHERE user_type = 'health_worker' ORDER BY fullname ASC";
$result = $conn->query($sql);
if ($result && $result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
        // Check if this user is already in the list (by name)
        $exists = false;
        ...
        if (!$exists) {
            $workers[] = [
                'name' => $row['name'],
                'title' => $row['specialty'] ?? 'Health Worker',
                'location' => 'Community Health Center',
                'specialty' => $row['specialty'] ?? 'General Health Support',
                'phone' => $row['phone'] ?? '',
                'email' => $row['email'] ?? '',
                'status' => 'Available',
                'image_url' => $row['avatar'] ?? "https://ui-avatars.com/api/?name=" . urlencode($row['name']) . "&background=3b82f6&color=fff"
            ];
        }
    }
}

// If no health workers in database, use default workers
if (count($workers) == 0) {
    $workers = [
        [
            'name' => 'Maria Clara, RM',
            'title' => 'Registered Midwife',
            'location' => 'Barangay Health Center - Poblacion',
            'specialty' => 'Maternal & Child Care',
            'phone' => '09123456789',
            'email' => '',
            'status' => 'Available',
            'image_url' => 'https://api.dicebear.com/7.x/avataaars/svg?seed=Maria&gender=female'
        ],
        [
            'name' => 'Juan Dela Cruz',
            'title' => 'Barangay Health Worker',
            'location' => 'Community Outreach - Purok 7',
            'specialty' => 'General Health Support',
            'phone' => '09123456780',
            'email' => '',
            'status' => 'Available',
            'image_url' => 'https://api.dicebear.com/7.x/avataaars/svg?seed=Juan&gender=male'
        ],
        [
            'name' => 'Dr. Jose Rizal, MD',
            'title' => 'General Physician',
            'location' => 'Municipal Health Office - Town Center',
            'specialty' => 'General Physician',
            'phone' => '09123456788',
            'email' => 'jose@malasakit.ph',
            'status' => 'Busy',
            'image_url' => 'https://api.dicebear.com/7.x/avataaars/svg?seed=DocRizal&gender=male'
        ],
        [
            'name' => 'Dr. Elena Adarna',
            'title' => 'Pediatric Specialist',
            'location' => 'Rural Health Unit - Sitio Maligaya',
            'specialty' => 'Pediatric Specialist',
            'phone' => '09123456781',
            'email' => '',
            'status' => 'Available',
            'image_url' => 'https://api.dicebear.com/7.x/avataaars/svg?seed=Elena&gender=female'
        ],
        [
            'name' => 'Antonio Luna, RND',
            'title' => 'Nutritionist-Dietitian',
            'location' => 'Community Center - Barangay Malinis',
            'specialty' => 'Nutritionist-Dietitian',
            'phone' => '09123456782',
            'email' => '',
            'status' => 'Available',
            'image_url' => 'https://api.dicebear.com/7.x/avataaars/svg?seed=Antonio&gender=male'
        ],
        [
            'name' => 'Luz Viminda',
            'title' => 'Senior Health Lead',
            'location' => 'Senior Health Lead - Purok 1',
            'specialty' => 'Elderly Care Lead',
            'phone' => '09123456783',
            'email' => '',
            'status' => 'In Field',
            'image_url' => 'https://api.dicebear.com/7.x/avataaars/svg?seed=Luz&gender=female'
        ]
    ];
}
?>
